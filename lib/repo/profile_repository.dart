...
import 'package:nex2u/models/update_alert_image_pic_request.dart';
...
class ProfileRepository {
  ...
  Future<bool?> updateAlert(
      UpdateAlertPicRequest payload, BuildContext context) async {
    try {
      final request = {
      ...
      final configService =
          Provider.of<ConfigurationViewModel>(context, listen: false);
      final response = await _apiClient.postWithoutJson(
          "/mdm-api/file-handler/uploadFarmlandAlertPic", // API endpoint
          request, // Convert request to JSON
          headers: headers,
          isJson: true, // Content-Type: application/json
          isMultipart: true);
